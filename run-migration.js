// Simple migration runner for profile_photos table
...
CREATE INDEX IF NOT EXISTS idx_profile_photos_position ON public.profile_photos(position);
CREATE INDEX IF NOT EXISTS idx_profile_photos_main ON public.profile_photos(is_main);

-- Apply updated_at trigger
DROP TRIGGER IF EXISTS handle_profile_photos_updated_at ON public.profile_photos;
...
BEGIN
  ...
  INSERT INTO public.profile_photos (user_id, photo_url, is_main, position)
...
async function runMigration() {
  try {
    console.log('Running profile_photos migration...');
    ...
    for (const statement of statements) {
      if (statement.trim()) {
        ...
        if (error) {
          ...
          const { error: rawError } = await supabase.from('sql').select('*').single();
